...
if __name__ == "__main__":
    ...
    for user in sorted(drives):
        for drive_info in sorted(drives[user]):
            ...
            usable_drives = process_db_file_for_individual_drives(
                db_file_path, camera_type
            )
            try:
                for session in usable_drives:
                    ...
                    print(
                        f"GNSS Start Time: {gnss_system_time[0]}, IMU Start Time: {imu_time[0]}"
                    )
                    ...
                    acc_x_aligned = align_sensor_with_reference(
                        imu_time, acc_x, gnss_system_time
                    )
                    acc_y_aligned = align_sensor_with_reference(
                        imu_time, acc_y, gnss_system_time
                    )
                    acc_z_aligned = align_sensor_with_reference(
                        imu_time, acc_z, gnss_system_time
                    )
                    gyro_x_aligned = align_sensor_with_reference(
                        imu_time, gyro_x, gnss_system_time
                    )
                    gyro_y_aligned = align_sensor_with_reference(
                        imu_time, gyro_y, gnss_system_time
                    )
                    gyro_z_aligned = align_sensor_with_reference(
                        imu_time, gyro_z, gnss_system_time
                    )
                    ...
                    acc_x_diff_lowpass = butter_lowpass_filter(
                        abs_padded_acc_x_diff, gnss_freq, cutoff_freq
                    )
                    acc_y_diff_lowpass = butter_lowpass_filter(
                        abs_padded_acc_y_diff, gnss_freq, cutoff_freq
                    )
                    acc_z_diff_lowpass = butter_lowpass_filter(
                        abs_padded_acc_z_diff, gnss_freq, cutoff_freq
                    )
                    gyro_x_diff_lowpass = butter_lowpass_filter(
                        abs_padded_gyro_x_diff, gnss_freq, cutoff_freq
                    )
                    gyro_y_diff_lowpass = butter_lowpass_filter(
                        abs_padded_gyro_y_diff, gnss_freq, cutoff_freq
                    )
                    gyro_z_diff_lowpass = butter_lowpass_filter(
                        abs_padded_gyro_z_diff, gnss_freq, cutoff_freq
                    )

                    # Gyro threshold +- 0.001, Accel threshold +- 0.001
                    acc_x_stopped = threshold_based_window_averaging(
                        acc_x_diff_lowpass, gnss_system_time, 1000, 0.002
                    )
                    acc_y_stopped = threshold_based_window_averaging(
                        acc_y_diff_lowpass, gnss_system_time, 1000, 0.002
                    )
                    acc_z_stopped = threshold_based_window_averaging(
                        acc_z_diff_lowpass, gnss_system_time, 1000, 0.002
                    )
                    gyro_x_stopped = threshold_based_window_averaging(
                        gyro_x_diff_lowpass, gnss_system_time, 1000, 0.001
                    )
                    gyro_y_stopped = threshold_based_window_averaging(
                        gyro_y_diff_lowpass, gnss_system_time, 1000, 0.001
                    )
                    gyro_z_stopped = threshold_based_window_averaging(
                        gyro_z_diff_lowpass, gnss_system_time, 1000, 0.001
                    )
                    ...
                    stationary_points = [
                        index
                        for index, val in enumerate(combined_gyro_accel)
                        if val == 0
                    ]
                    ...
                    create_map_with_highlighted_indexes(
                        lats, lons, stationary_points, "imu_stationary.html"
                    )
                    plt.show()
